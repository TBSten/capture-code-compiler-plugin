...
実行可能なサンプルが [`samples/`](samples/) 配下にある:

- [`samples/jvm-sample/`](samples/jvm-sample/) — 最小 JVM 構成と 5 ケースの
  cookbook (marker / filler / user-defined params / 全宣言ターゲット /
  `@file:` キャプチャ)。
- [`samples/kmp-sample/`](samples/kmp-sample/) — test sourceset 完結方式の
  KMP 構成 (`jvm` / `js` / `linuxX64` / `mingwX64`)。
...
- [docs/versioning.md](docs/versioning.md) — semver ポリシー、 pre-1.0 期の
  保証範囲、 release artefact、 release 間の VERSION_NAME 運用方針。
...
Apple target は `-PenableAppleTargets=true` の opt-in 制御を採用しているため、 Xcode を持たない環境 (ローカル開発 / デフォルト CI runner) でも E2E build が成立する。 完全な Apple matrix 検証は macos-latest 系 runner を追加する形で後続フェーズで対応する想定。

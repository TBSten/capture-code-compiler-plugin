...
/**
 * Kotlin 2.0.x baseline 向けの **Logic B-fir** checker (式 annotation の site collector)。
 *
 * task-072 で `:compiler-plugin` main module の `CaptureCodeFirExpressionSiteCollector` を
 * compat-k200 layer に移動した版。 `FirLiteralExpression<*>` (型パラメータ付き、 K2.0 baseline)
 * を直接 dispatch して `value` を読み取る。 K2.1+ では型パラメータが削除されているため、
 * compat-k210 側で再実装する。
 */

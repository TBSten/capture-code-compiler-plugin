...
public class CompatContextImpl : CompatContext {
    ...
    override fun diagnosticFactory(id: String): DiagnosticFactoryRef? = K200Diagnostics.MAP[id]
    ...
    override fun transformCallsInFile(
        ...
        file.transformChildrenVoid(K200CallTransformer(onCall))
    }
    ...
    // -- task-0.2.0-cifix-ir: IR factory drift dispatchers (K2.0 baseline) --
    //
    // K2.0 baseline では `IrVarargImpl(...)`, `IrConstImpl(...)`, `IrConstImpl.string(...)` /
    // `.int(...)`, `IrGetEnumValueImpl(...)` の top-level builders がそれぞれ
    // 個別 `*Kt` host class (e.g. `IrVarargImplKt`, `IrConstImplKt`, `IrGetEnumValueImplKt`)
    // にコンパイルされる。 後続 baseline で同 builders が consolidated `BuildersKt` に
    // 集約された結果、 main bytecode の `INVOKESTATIC ...IrVarargImplKt.IrVararg(...)` が
    // K2.1+ runtime で `ClassNotFoundException` を起こすため、 各 compat-kXXX 経由で
    // それぞれの baseline の builder host を再 link する。
    ...
    override fun newIrConstPrimitive(
        ...
        // K2.0 baseline: `IrConstKind<T>` (generic). SPI `Any` erasure を baseline 固有の
        // `IrConstKind<Any?>` に戻して 5-arg builder へ。
        startOffset = startOffset,
        endOffset = endOffset,
        type = type,
    ...
    public object K200Diagnostics {
         ...
         *
         * task-132: 戻り値型を [DiagnosticFactoryRef] に変更。 各 entry は
         * `KtDiagnosticFactory0` か `KtDiagnosticFactory1<String>` の sum を sealed
         * 経由で表現する。 新 entry 追加時は `Zero(...)` / `OneString(...)` のいずれかを
         * 必ず選ぶ必要があり、 型ミスマッチが compile-time に弾かれる。
         */

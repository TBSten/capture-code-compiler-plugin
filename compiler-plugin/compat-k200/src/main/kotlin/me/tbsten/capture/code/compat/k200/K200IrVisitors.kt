 ...
 *
 * K2.0 baseline では `IrElementVisitorVoid` は **interface** で、 default の
 * `visitElement` 経由で `acceptChildrenVoid(this)` が呼ばれる構造。 visitor 自身に
 * state を持たず、 4 つの callback だけで dispatch する pattern。
 *
 * 各 `visit*` override は **callback invoke → `acceptChildrenVoid(this)` で再帰** の
 * 2 ステップ。 acceptChildrenVoid を忘れると nested declaration を見落とすので注意。
 */
internal class K200CallbackVisitor(
 ...
 *
 * children を super で transform した後 callback に渡すので、 nested rewrite が正しく
 * compose される。 callback 戻り値が non-null なら置換、 null なら super の結果 (= 元
 * call) をそのまま返す。
 */
internal class K200CallTransformer(

 ...
 *
 * 各 compat-kXXX 層の diagnostic factory (`K200CaptureCodeDiagnostics` 等) の renderer 文面を
 ...
 * 各 compat-kXXX layer の diagnostic factory が参照するため、 共有モジュールへ昇格した。
 */
public object CaptureCodeDiagnosticMessages {
     ...
     * 引数: `{0}` = parameter name。
     */
    public val MARKER_PARAMETER_TYPE_INVALID: BilingualMessage = BilingualMessage(
     ...
     * 引数: `{0}` = parameter name。
     */
    public val MARKER_FILLER_REQUIRES_DEFAULT: BilingualMessage = BilingualMessage(
    ...
    public val MARKER_IS_EXPECT: BilingualMessage = BilingualMessage(
     ...
     * 引数: `{0}` = T の FQN。
     */
    public val CAPTUREDSOURCES_T_NOT_CAPTURE_CODE: BilingualMessage = BilingualMessage(
     ...
     */
    public fun render(message: BilingualMessage): String =

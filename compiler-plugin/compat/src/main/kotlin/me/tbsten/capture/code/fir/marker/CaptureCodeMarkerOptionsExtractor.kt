 ...
 * [CaptureCodeMarkerOptions] を構築する drift-free ヘルパ。
 *
 ...
 * (= override なし) として扱う。
 *
 * **配置**: task-072 で `:compiler-plugin` main module の `CaptureCodeFirMarkerService` 内の
 * `FirAnnotation.extractMarkerOptions()` private 関数から `:compiler-plugin:compat` に切り出した。
 * FIR API は本処理範囲では drift しないため (FirGetClassCall / FirPropertyAccessExpression /
 * FirQualifiedAccessExpression / `toResolvedCallableSymbol()` / `FirCallableSymbol.callableId`
 * のすべてが 2.0.x / 2.1.x / 2.2.x で互換)、 compat module 共有版として 1 箇所に集約する。
 */
public object CaptureCodeMarkerOptionsExtractor {
     ...
     */
    public fun extract(annotation: FirAnnotation): CaptureCodeMarkerOptions {

// Kotlin 2.2.x で `putValueArgument` / `putTypeArgument` / `valueParameters` 等が
// `@Deprecated(level = DeprecationLevel.ERROR)` 化された。 IR builder の MutableList 移行は
// patch 内でも継続中のため、 旧 API 呼び出しで bytecode を維持しつつ deprecation を抑止する。
...
import me.tbsten.capture.code.compat.k220.filler.CaptureKindFillerBuilder
import me.tbsten.capture.code.compat.k220.filler.FillerBuilder
import me.tbsten.capture.code.compat.k220.filler.SourceFillerBuilder
import me.tbsten.capture.code.compat.k220.filler.SourceLocationFillerBuilder
import me.tbsten.capture.code.compat.k220.userargs.UserArgIrBuilder
import me.tbsten.capture.code.compat.k220.userargs.UserArgPrimitiveIrBuilder
 ...
 * [K220CapturedSourcesCollector] が収集した [K220CapturedSiteData] のリストから組み立てた
 ...
 * - **SourceNormalizer の wire up**: 生 source 取得は [K220CapturedSourcesCollector] 内で
 ...
 */
internal object K220CapturedSourcesRewriter {
    ...
    fun rewriteCapturedSourcesCall(
        original: IrCall,
        markerFqn: String,
        siteData: List<K220CapturedSiteData>,

 ...
 * - 発見したサイトを [CapturedSite] にして [K220CapturedSourcesTransformer.capturedSiteData] に蓄積
 ...
 */
internal fun runK220IrTransform(
    ...
    // なっていることが保証される。
    val transformer = K220CapturedSourcesTransformer(pluginContext, config)

    // パス 1: 全 file を visit して capturedSiteData を埋める (rewrite はしない)。
    for (file in moduleFragment.files) {
        val collector = K220CapturedSourcesCollector(file, config)
 ...
 * 全 file collect → パス 2: rewrite) になっており、 collect は [runK220IrTransform] 側で行う。
 ...
 */
internal class K220CapturedSourcesTransformer(
    private val pluginContext: IrPluginContext,
    private val config: CaptureCodePluginConfig,
) : IrElementTransformerVoid() {

    val capturedSiteData: MutableList<K220CapturedSiteData> = mutableListOf()
    ...
    override fun visitCall(expression: IrCall): IrExpression {
        ...
        val rewritten = K220CapturedSourcesRewriter.rewriteCapturedSourcesCall(

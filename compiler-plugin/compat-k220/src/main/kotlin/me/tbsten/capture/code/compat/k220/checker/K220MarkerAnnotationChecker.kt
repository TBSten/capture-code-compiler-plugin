 ...
 * ([K220MarkerAnnotationCheckerShim]) で `FirRegularClassChecker` を継承し、 本オブジェクトに
...
public object K220MarkerAnnotationCheckerLogic {
    ...
    public fun run(
        ...
        if (declaration.isExpect) {
            reporter.reportOn(source, K220CaptureCodeDiagnostics.CC_MARKER_IS_EXPECT, context)
        }
    ...
    private fun checkVisibility(
        ...
        if (visibility != Visibilities.Internal && visibility != Visibilities.Private) {
            reporter.reportOn(
                declaration.source,
                K220CaptureCodeDiagnostics.CC_MARKER_VISIBILITY_VIOLATION,
                context,
            )
        }
    }
    ...
    private fun checkRetention(
        ...
        if (declaration.getRetention(context.session) != AnnotationRetention.SOURCE) {
            reporter.reportOn(
                declaration.source,
                K220CaptureCodeDiagnostics.CC_MARKER_RETENTION_VIOLATION,
                context,
            )
        }
    }
    ...
    private fun checkTargetSites(
        ...
        if (targetAnnotation == null) {
            reporter.reportOn(
                declaration.source,
                K220CaptureCodeDiagnostics.CC_MARKER_TARGET_EMPTY,
                context,
            )
            return
        }

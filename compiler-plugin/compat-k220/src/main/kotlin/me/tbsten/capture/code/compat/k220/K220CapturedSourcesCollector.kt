 ...
 */
internal data class K220CapturedSiteData(
     ...
     * [me.tbsten.capture.code.compat.k220.userargs.UserArgIrBuilder.buildOrDefault] の
 ...
 * [collectFileAnnotations] を [K220IrInjector] パス 1 から **declaration 走査と並行で** 呼び出す
 ...
 */
internal class K220CapturedSourcesCollector(
     ...
     * [K220IrInjector] パス 1 から **declaration 走査とは独立に** 呼ぶ。declaration 走査と
    ...
    fun collectFileAnnotations() {
        ...
        for ((markerFqn, markerCall) in fileAnnotations) {
            ...
            capturedSiteData += K220CapturedSiteData(
                site = CapturedSite(
                    markerFqn = markerFqn,
                    source = source,
     ...
     * `endLine`) も合わせて埋める。 後段 ([K220CapturedSourcesRewriter]) で

...
import me.tbsten.capture.code.compat.k240.CompatContextImpl
import me.tbsten.capture.code.compat.k240.k240Compat
...
/**
 * Kotlin 2.4.0-RC baseline 向けの `FirAdditionalCheckersExtension` 群 + 各 logic dispatcher。
 *
 * task-124 で旧 `K240CaptureCodeMarkerClassChecker.kt` / `K240MarkerAnnotationChecker.kt` /
 * `K240CapturedSourcesCallChecker.kt` / `K240ExpressionSiteCollector.kt` の 4 dispatcher
 * file を本 file に集約。 各 `K240*CheckerLogic` object は Java shim
 * (`K240*Shim.java`) から `INSTANCE.run(...)` で呼ばれる。
 *
 * task-119 follow-up: 各 dispatcher で `CompatContextImpl()` を 3 個 new していた状態を
 * module-scoped singleton ([k240Compat]) で共有する形に統一。
 */
...
public object K240CaptureCodeMarkerClassCheckerLogic {
    ...
    public fun run(
        ...
        discoverMarkerClass(context, declaration, k240Compat)
    }
}

/** Logic F — marker annotation 検査 + warning chain (CC_MARKER_OVERRIDE_NO_EFFECT)。 */
public object K240MarkerAnnotationCheckerLogic {
    ...
    private val diagnostics = object : ValidateMarkerAnnotation.Diagnostics {
        override val markerIsExpect: KtDiagnosticFactory0 =
            CompatContextImpl.K240Diagnostics.CC_MARKER_IS_EXPECT
        override val markerParameterTypeInvalid: KtDiagnosticFactory1<String> =
            CompatContextImpl.K240Diagnostics.CC_MARKER_PARAMETER_TYPE_INVALID
        override val markerFillerRequiresDefault: KtDiagnosticFactory1<String> =
            CompatContextImpl.K240Diagnostics.CC_MARKER_FILLER_REQUIRES_DEFAULT
    }
    private val warningDiagnostics = object : WarnIfOverrideNoEffect.Diagnostics {
        override val markerOverrideNoEffect: KtDiagnosticFactory1<String> =
            CompatContextImpl.K240Diagnostics.CC_MARKER_OVERRIDE_NO_EFFECT
    }
    ...
    public fun run(
        ...
        logic(context, reporter, declaration, k240Compat, diagnostics)
...
public object K240CapturedSourcesCallCheckerLogic {
    ...
    private val diagnostics = object : ValidateCapturedSourcesCall.Diagnostics {
        override val capturedSourcesTNotCaptureCode: KtDiagnosticFactory1<String> =
            CompatContextImpl.K240Diagnostics.CC_CAPTUREDSOURCES_T_NOT_CAPTURE_CODE
        override val capturedSourcesTIsTypeParameter: KtDiagnosticFactory1<String> =
            CompatContextImpl.K240Diagnostics.CC_CAPTUREDSOURCES_T_IS_TYPE_PARAMETER
    }
    ...
    public fun run(
        ...
        logic(context, reporter, expression, k240Compat, diagnostics)
    }
}

/** Logic B-fir — expression site 収集 (file-level annotation も含む)。 */
public object K240ExpressionSiteCollectorLogic {
    ...
    public fun run(
        ...
        logic(context, reporter, expression, k240Compat)
        blockSiteLogic(context, reporter, expression, k240Compat)
    }
}
...
internal class K240MarkerCheckersExtension(session: FirSession) : FirAdditionalCheckersExtension(session) {
    override val declarationCheckers: DeclarationCheckers = object : DeclarationCheckers() {
        override val regularClassCheckers: Set<FirDeclarationChecker<FirRegularClass>> =
            setOf(K240CaptureCodeMarkerClassCheckerShim.INSTANCE)
    }
}

internal class K240MarkerAnnotationCheckersExtension(session: FirSession) : FirAdditionalCheckersExtension(session) {
    override val declarationCheckers: DeclarationCheckers = object : DeclarationCheckers() {
        override val regularClassCheckers: Set<FirDeclarationChecker<FirRegularClass>> =
            setOf(K240MarkerAnnotationCheckerShim.INSTANCE)
    }
}

internal class K240CapturedSourcesCallCheckersExtension(session: FirSession) : FirAdditionalCheckersExtension(session) {
    override val expressionCheckers: ExpressionCheckers = object : ExpressionCheckers() {
        override val functionCallCheckers: Set<FirExpressionChecker<FirFunctionCall>> =
            setOf(K240CapturedSourcesCallCheckerShim.INSTANCE)
    }
}

internal class K240ExpressionAnnotationCheckersExtension(session: FirSession) : FirAdditionalCheckersExtension(session) {
    override val expressionCheckers: ExpressionCheckers = object : ExpressionCheckers() {
        override val basicExpressionCheckers: Set<FirExpressionChecker<FirStatement>> =
            setOf(K240ExpressionSiteCollectorShim.INSTANCE)
    }
}

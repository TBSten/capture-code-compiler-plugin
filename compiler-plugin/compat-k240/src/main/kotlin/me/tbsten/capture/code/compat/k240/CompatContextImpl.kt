...
// 削除済 (deprecated ERROR を超えて symbol 自体消滅)。 内部 shim (`K240IrApiShims.kt`) を
...
import me.tbsten.capture.code.compat.k240.checker.K240CapturedSourcesCallCheckersExtension
import me.tbsten.capture.code.compat.k240.checker.K240ExpressionAnnotationCheckersExtension
import me.tbsten.capture.code.compat.k240.checker.K240MarkerAnnotationCheckersExtension
import me.tbsten.capture.code.compat.k240.checker.K240MarkerCheckersExtension
import me.tbsten.capture.code.compat.k240.checker.K240RendererMapShim
 ...
 */
internal val k240Compat: CompatContext = CompatContextImpl()

/**
 * Kotlin 2.4.x (2.4.0-RC 以降) 向けの [CompatContext] 実装。
 *
 * task-076: compat-k230 (Kotlin 2.3.x baseline) を template に mechanical copy + rename
 * したもの。 2.3.x → 2.4.x の追加 native API drift は本 module で吸収する。 現時点で
 * 確認している方針:
 *
 * - **compile baseline = Kotlin 2.4.0 (stable)**: 2.4.0 stable release 後に
 *   `kotlin-k240` catalog entry を `2.4.0-RC` から bump 済 (module 名も `compat-k240rc`
 *   → `compat-k240` に rename)。 2.4.0 / 2.4.10 (stable) と 2.4.20-RC (pre-release) は
 *   CI matrix で実機検証済。
 * - **FIR checker `check()` signature**: 2.2.x 以降の `context(...) fun check(D)`
 *   (Java shim 経由) は 2.4.x でもそのまま有効。
 * - **IR API drift**: `IrMemberAccessExpression.putValueArgument` 等の deprecated migration
 *   は 2.4.x でも継承。 大きな drift が観測されたら個別 ticket で対応。
 * - **Factory.minVersion = "2.4.0-RC"**: [KotlinToolingVersion] の pre-release tier
 *   (Maturity.RC) を介して比較される。 consumer kotlin = 2.4.0-RC の時に
 *   compat-k230 (minVersion 2.3.0, STABLE) より上位として選ばれ、 stable の
 *   2.4.0 / 2.4.10 / 2.4.20-RC も同様に本 module が dispatch される
 *   (`2.4.0-RC < 2.4.0 < 2.4.10 < 2.4.20-RC`)。 baseline を 2.4.0 stable に bump した
 *   後も minVersion を RC のまま据え置くのは、 2.4.0-RC を使う consumer を
 *   compat-k230 (= 確実に drift する) へ落とさないための意図的な選択。
 *
...
public class CompatContextImpl : CompatContext {
    ...
    // owned by `CaptureCodeIrExtension` (main module). `K240IrTransform` +
    // `K240CapturedSourcesCollector` + `K240CapturedSourcesRewriter` are now dead code
    ...
    override fun firAdditionalCheckersExtensions():
        ...
        ::K240MarkerCheckersExtension,
        ::K240MarkerAnnotationCheckersExtension,
        ::K240CapturedSourcesCallCheckersExtension,
        ::K240ExpressionAnnotationCheckersExtension,
    )
    ...
    override fun diagnosticFactory(id: String): DiagnosticFactoryRef? = K240Diagnostics.MAP[id]
    ...
    //
    // `K240IrApiShims.kt` で `putArgumentSafe` / `getArgumentSafe` / `setTypeArgumentSafe` /
    ...
    override fun walkIrTree(
        ...
        moduleFragment.acceptChildrenVoid(
            K240CallbackVisitor(onClass, onSimpleFunction, onProperty, onTypeAlias),
        )
    }
    ...
    override fun transformCallsInModule(
        ...
        moduleFragment.transformChildrenVoid(K240CallTransformer(onCall))
    }
    ...
    override fun transformCallsInFile(
        ...
        file.transformChildrenVoid(K240CallTransformer(onCall))
    }
    ...
    //     ため、 K2.4-RC runtime で `ClassNotFoundException` を起こすが、 compat-k240 を経由
     ...
     * 旧 `checker/K240CaptureCodeDiagnostics.kt` から本 nested object に集約)。
     *
     ...
     */
    public object K240Diagnostics : KtDiagnosticsContainer() {
        ...
        public val CC_CAPTUREDSOURCES_T_NOT_CAPTURE_CODE: KtDiagnosticFactory1<String> =
            KtDiagnosticFactory1(
                ...
                rendererFactory = K240CaptureCodeDefaultMessages,
            )
        ...
        override fun getRendererFactory(): BaseDiagnosticRendererFactory = K240CaptureCodeDefaultMessages
        ...
        private object K240CaptureCodeDefaultMessages : BaseDiagnosticRendererFactory() {
            ...
            override val MAP: KtDiagnosticFactoryToRendererMap by lazy {
                K240RendererMapShim.create("CaptureCode").apply {
                    put(
                        CC_MARKER_PARAMETER_TYPE_INVALID,

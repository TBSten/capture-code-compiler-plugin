// ----------------------------------------------------------------------------
// この module は **publish しない** (internal compat layer for Kotlin 2.4.x)。
...
kotlin {
    compilerOptions {
        ...
        // root KGP (Kotlin 2.0.0) で kotlin-compiler-embeddable-2.4.0 を読むと、
        // 2.4.0 metadata format (binary version: 2.4.0) が 2.0.0 expected を超えるため
        // metadata version check で fail する。 本 module は **bundled-only**
        // (= ServiceLoader 経由でしか参照されない) であり、 main module 経由で
        // 2.4.0 シンボルを直接 import することは無いため、 metadata check を
        // skip しても安全。
        ...
        // に変わって以降、 2.4.0 でも同じ shape のため、 root KGP (Kotlin 2.0.0)
...
dependencies {
    // Kotlin 2.4.0 native API でビルドする。 compileOnly のみなので、 published jar には
    // kotlin-compiler-embeddable を引き込まない (shadowJar も同様)。
    compileOnly(libs.kotlin.compiler.embeddable.k240)
    ...
    // task-122: 本 module の `K240Diagnostics` 内 renderer は main module の English-only
    // SSoT (`MarkerAnnotationErrors` / `CapturedSourcesCallErrors`) を `<clinit>` で参照する。
    // `K240CaptureCodeDiagnosticsStaticInitTest` が直接 diagnostic property + renderer factory MAP
    ...
    // task-076: compat-k240 専用 unit test (ServiceLoader-based sanity)。
    // kctfork は意図的に使わず pure JVM test に留める。 こうすることで
    // kctfork transitive embeddable と consumer Kotlin (matrix bumped to 2.4.0+)
    // の drift 問題を本 module の test では完全回避できる。
    // testRuntime に kotlin-compiler-embeddable-k240 (= 2.4.0 固定) を入れて
    // K240CompatContextImpl が ServiceLoader 経由でロードされる際に
    // 必要な FIR / IR symbol を解決可能にする。
    testImplementation(libs.kotlin.compiler.embeddable.k240)

...
import me.tbsten.capture.code.compat.k240rc.k240RcCompat
 ...
 *
 * task-124 で旧 `K240RcCaptureCodeMarkerClassChecker.kt` / `K240RcMarkerAnnotationChecker.kt` /
 * `K240RcCapturedSourcesCallChecker.kt` / `K240RcExpressionSiteCollector.kt` の 4 dispatcher
 * file を本 file に集約。 各 `K240Rc*CheckerLogic` object は Java shim
 * (`K240Rc*Shim.java`) から `INSTANCE.run(...)` で呼ばれる。
 *
 * task-119 follow-up: 各 dispatcher で `CompatContextImpl()` を 3 個 new していた状態を
 * module-scoped singleton ([k240RcCompat]) で共有する形に統一。
 */
...
public object K240RcCaptureCodeMarkerClassCheckerLogic {
    ...
    public fun run(
        ...
        discoverMarkerClass(context, declaration, k240RcCompat)
    }
}
...
public object K240RcMarkerAnnotationCheckerLogic {
    ...
    private val diagnostics = object : ValidateMarkerAnnotation.Diagnostics {
        override val markerIsExpect: KtDiagnosticFactory0 =
            CompatContextImpl.K240RcDiagnostics.CC_MARKER_IS_EXPECT
        override val markerParameterTypeInvalid: KtDiagnosticFactory1<String> =
            CompatContextImpl.K240RcDiagnostics.CC_MARKER_PARAMETER_TYPE_INVALID
        override val markerFillerRequiresDefault: KtDiagnosticFactory1<String> =
            CompatContextImpl.K240RcDiagnostics.CC_MARKER_FILLER_REQUIRES_DEFAULT
    }
    private val warningDiagnostics = object : WarnIfOverrideNoEffect.Diagnostics {
        override val markerOverrideNoEffect: KtDiagnosticFactory1<String> =
            CompatContextImpl.K240RcDiagnostics.CC_MARKER_OVERRIDE_NO_EFFECT
    }
    ...
    public fun run(
        ...
        logic(context, reporter, declaration, k240RcCompat, diagnostics)
...
public object K240RcCapturedSourcesCallCheckerLogic {
    ...
    public fun run(
        ...
        logic(context, reporter, expression, k240RcCompat, diagnostics)
    }
}
...
public object K240RcExpressionSiteCollectorLogic {
    ...
    public fun run(
        ...
        logic(context, reporter, expression, k240RcCompat)
    }
}

// task-076: Kotlin 2.4.0-RC で `getTypeArgument(Int)` が削除された。 [getTypeArgumentSafe]
// (本 module 内 shim) で `typeArguments[i]` を呼び出す形に書き換える。
 ...
 * - 発見したサイトを [CapturedSite] にして [K240RcCapturedSourcesTransformer.capturedSiteData] に蓄積
 ...
 */
internal fun runK240RcIrTransform(
    ...
    // なっていることが保証される。
    val transformer = K240RcCapturedSourcesTransformer(pluginContext, config)

    // パス 1: 全 file を visit して capturedSiteData を埋める (rewrite はしない)。
    for (file in moduleFragment.files) {
        val collector = K240RcCapturedSourcesCollector(file, config)
 ...
 * 全 file collect → パス 2: rewrite) になっており、 collect は [runK240RcIrTransform] 側で行う。
 ...
 */
internal class K240RcCapturedSourcesTransformer(
    ...
    override fun visitCall(expression: IrCall): IrExpression {
        ...
        val rewritten = K240RcCapturedSourcesRewriter.rewriteCapturedSourcesCall(
    ...
    private fun IrCall.isCapturedSourcesCall(): Boolean =
        symbol.owner.fqNameWhenAvailable?.asString() == K240RcCapturedSourcesRewriter.CAPTURED_SOURCES_FQN
    ...
    private fun IrCall.markerTypeArgumentOrNull(): String? {
        val typeArg = getTypeArgumentSafe(0) ?: return null

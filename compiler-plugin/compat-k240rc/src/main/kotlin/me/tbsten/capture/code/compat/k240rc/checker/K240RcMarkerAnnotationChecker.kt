...
public object K240RcMarkerAnnotationCheckerLogic {
    ...
    private fun checkVisibility(
        ...
        if (visibility != Visibilities.Internal && visibility != Visibilities.Private) {
            reporter.reportOn(
                declaration.source,
                K240RcCaptureCodeDiagnostics.CC_MARKER_VISIBILITY_VIOLATION,
                context,
            )
        }
    }
    ...
    private fun checkRetention(
        ...
        if (declaration.getRetention(context.session) != AnnotationRetention.SOURCE) {
            reporter.reportOn(
                declaration.source,
                K240RcCaptureCodeDiagnostics.CC_MARKER_RETENTION_VIOLATION,
                context,
            )
        }
    }
    ...
    private fun checkTargetSites(
        ...
        if (targetAnnotation == null) {
            reporter.reportOn(
                declaration.source,
                K240RcCaptureCodeDiagnostics.CC_MARKER_TARGET_EMPTY,
                context,
            )
            return
        }

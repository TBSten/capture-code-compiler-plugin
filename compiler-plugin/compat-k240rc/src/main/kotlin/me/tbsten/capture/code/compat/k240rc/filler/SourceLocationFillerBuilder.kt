// task-076: Kotlin 2.4.0-RC で `putValueArgument` / `valueParameters` が削除された。
// 内部 shim [putArgumentSafe] と `IrFunction.nonDispatchParameters` (IrUtilsKt) を使う。
...
internal class SourceLocationFillerBuilder(
    ...
    override fun build(site: CapturedSite, config: CaptureCodePluginConfig): IrExpression {
        ...
        return IrConstructorCallImpl.fromSymbolOwner(
            ...
            putArgumentSafe(packageNameIndex, stringConst(site.packageFqn))
            putArgumentSafe(filePathIndex, stringConst(site.filePath))
            putArgumentSafe(startLineIndex, intConst(effectiveStartLine))
            putArgumentSafe(endLineIndex, intConst(effectiveEndLine))
        }
    }
    ...
    companion object {
        ...
        fun resolve(pluginContext: IrPluginContext): SourceLocationFillerBuilder? {
            ...
            val locationConstructor = locationSymbol.owner.constructors.firstOrNull()?.symbol
                ?: return null
            val params: List<IrValueParameter> = locationConstructor.owner.nonDispatchParameters

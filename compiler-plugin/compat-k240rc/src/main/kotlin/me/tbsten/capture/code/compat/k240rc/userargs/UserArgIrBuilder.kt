// task-076: Kotlin 2.4.0-RC drift — `IrConstructorCall.getValueArgument(Int)` 削除。
// 内部 shim [getArgumentSafe] (`arguments[i]` 相当) を使う。 `deepCopyWithSymbols` も同様に
// 削除されており、 後継 `deepCopyWithoutPatchingParents` がある。 ただし 2.4.0-RC では
// 当該 helper の API が更に変わっている可能性があるため、 本 file では現状の API に
// 合わせて差し替える。
@file:Suppress("DEPRECATION", "DEPRECATION_ERROR")
@file:OptIn(org.jetbrains.kotlin.DeprecatedForRemovalCompilerApi::class)

package me.tbsten.capture.code.compat.k240rc.userargs

import me.tbsten.capture.code.compat.k240rc.getArgumentSafe
 ...
 * IR 化するための helper。
 *
 * filler 自動値埋め ([me.tbsten.capture.code.compat.k240rc.filler.FillerBuilder]) との境界:
 ...
 * annotation でも **IR まで `IrConstructorCall` が残る** (式 annotation は残らないが、
 * 宣言 annotation は残ることを確認済)。 このため、 collector が marker `IrConstructorCall` を
 * 保持しておけば、 rewriter は `getValueArgument(i)` で直接ユーザの式を取り出せる。
 * FIR storage を追加するより遥かにシンプル。 design §5 B-fir はあくまで「式 annotation 用」の
 * 経路として残し、 宣言 annotation では IR 直接ルートを採用する。
 *
 * ## deepCopy の必要性
 *
...
internal object UserArgIrBuilder {
     ...
     *
     * [markerCall] は nullable。 EXPRESSION 起源 (式 annotation) では IR phase で marker
     * `IrConstructorCall` が残らないため、 markerCall == null の場合は **直接** default 値経路を使う。
     * ユーザ定義 parameter の **動的値** (primitive など) は別 builder ([UserArgPrimitiveIrBuilder]) で扱う想定。
     * 現在は「ケース #7 / #67 = filler のみが入った marker」を主にサポートするため、
    ...
    fun buildOrDefault(
        ...
        if (markerCall != null) {
            val userExpr = markerCall.getArgumentSafe(parameterIndex)

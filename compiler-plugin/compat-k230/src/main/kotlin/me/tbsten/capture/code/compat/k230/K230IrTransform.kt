// Kotlin 2.2.x で `getTypeArgument` 等が `@Deprecated(level = ERROR)` 化された。 旧 API 呼び出しで
// 2.2.x runtime も透過動作するため、 deprecation を file 単位で抑止する。
 ...
 *   (property / class / object / function / typealias の 5 種をサポート)
 * - 発見したサイトを [CapturedSite] にして [K230CapturedSourcesTransformer.capturedSiteData] に蓄積
 * - `capturedSources<T>()` 呼び出しを `listOf(T(source = Source(...)))` へ書き換える
 *   (filler 未指定 marker の場合は 0-arg `T()` の list literal)
 *
 * Metro pattern で `IrInjector` interface を廃止し、 [CompatContextImpl.transformIr] に
 * 統合されたため、 本関数は単独 entry point として呼ばれる。 内部の 2 パス transform (collect →
 * rewrite) ロジックは従来と同一。
 *
 * 詳細な順序は `compiler-plugin-design.md` §6 Phase ordering 参照。
 */
internal fun runK230IrTransform(
    ...
    // なっていることが保証される。
    val transformer = K230CapturedSourcesTransformer(pluginContext, config)

    // パス 1: 全 file を visit して capturedSiteData を埋める (rewrite はしない)。
    for (file in moduleFragment.files) {
        val collector = K230CapturedSourcesCollector(file, config)
 ...
 *
 * **collect は本 transformer の責務ではない**。 transform は 2 パス構成 (パス 1:
 * 全 file collect → パス 2: rewrite) になっており、 collect は [runK230IrTransform] 側で行う。
 * 本 transformer は [capturedSiteData] を **外部から populate された read-only スナップショット**
 * として扱う。
 *
 * 詳細は `compiler-plugin-design.md` §5 Logic B/C/D/H、 §6 Phase ordering 参照。
 */
internal class K230CapturedSourcesTransformer(
    private val pluginContext: IrPluginContext,
    private val config: CaptureCodePluginConfig,
) : IrElementTransformerVoid() {

    val capturedSiteData: MutableList<K230CapturedSiteData> = mutableListOf()
    ...
    override fun visitCall(expression: IrCall): IrExpression {
        ...
        val rewritten = K230CapturedSourcesRewriter.rewriteCapturedSourcesCall(
    ...
    private fun IrCall.isCapturedSourcesCall(): Boolean =
        symbol.owner.fqNameWhenAvailable?.asString() == K230CapturedSourcesRewriter.CAPTURED_SOURCES_FQN

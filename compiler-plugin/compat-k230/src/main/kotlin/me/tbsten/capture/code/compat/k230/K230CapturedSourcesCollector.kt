 ...
 */
internal data class K230CapturedSiteData(
     ...
     * [me.tbsten.capture.code.compat.k230.userargs.UserArgIrBuilder.buildOrDefault] の
 ...
 * [collectFileAnnotations] を [K230IrInjector] パス 1 から **declaration 走査と並行で** 呼び出す
 ...
 */
internal class K230CapturedSourcesCollector(
     ...
     * [K230IrInjector] パス 1 から **declaration 走査とは独立に** 呼ぶ。declaration 走査と
    ...
    fun collectFileAnnotations() {
        ...
        for ((markerFqn, markerCall) in fileAnnotations) {
            ...
            capturedSiteData += K230CapturedSiteData(
                site = CapturedSite(
                    markerFqn = markerFqn,
                    source = source,
     ...
     * `endLine`) も合わせて埋める。 後段 ([K230CapturedSourcesRewriter]) で
    ...
    private fun expandStartToCoverModifierAndAnnotationLines(fullText: String, startOffset: Int): Int {
        ...
        //          current を lineStart に移動。 そうでない場合は同一行を遡らせない
        //          (expression-only EXPRESSION 起源 site の declaration 全体化を防ぐ)。
        ...
        //          到達した時点で停止。 Phase 1 で行頭に動かしていない場合は遡らない。

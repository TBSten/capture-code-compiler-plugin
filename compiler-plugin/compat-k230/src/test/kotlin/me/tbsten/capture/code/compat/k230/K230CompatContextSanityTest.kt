...
class K230CompatContextSanityTest : StringSpec({
    ...
    // dynamically but cannot instantiate k230 in a K2.0 baseline classpath, so
    // we verify the K230 MAP locally here (kctfork-free, baseline K2.3-only).
    //
    ...
    "K230 diagnosticFactory exposes the full 12-id set with matching wrap kinds" {
    ...
    "K230 diagnosticFactory returns null for unknown ids (silent no-op contract)" {
        val ctx = CompatContextImpl()

...
import me.tbsten.capture.code.compat.k202.filler.CaptureKindFillerBuilder
import me.tbsten.capture.code.compat.k202.filler.FillerBuilder
import me.tbsten.capture.code.compat.k202.filler.SourceFillerBuilder
import me.tbsten.capture.code.compat.k202.filler.SourceLocationFillerBuilder
import me.tbsten.capture.code.compat.k202.userargs.UserArgIrBuilder
import me.tbsten.capture.code.compat.k202.userargs.UserArgPrimitiveIrBuilder
 ...
 * [K202CapturedSourcesCollector] が収集した [K202CapturedSiteData] のリストから組み立てた
 ...
 *   クラス群に切り出している。`buildMarkerInstance` は marker constructor の各 parameter を順に
 ...
 * - **SourceNormalizer の wire up**: 生 source 取得は [K202CapturedSourcesCollector] 内で
 ...
 * `IrValueParameter.defaultValue` を使う。 design §5 Logic H / §7.9 参照。
 */
internal object K202CapturedSourcesRewriter {
    ...
    fun rewriteCapturedSourcesCall(
        original: IrCall,
        markerFqn: String,
        siteData: List<K202CapturedSiteData>,
     ...
     *
     * Kotlin の primary constructor は **value argument を全部 explicit に指定しなければ instance を
     * 生成できない** ため、 filler 以外のユーザ定義 parameter も含めて全 parameter を埋める。
     */
    private fun buildMarkerInstance(
        data: K202CapturedSiteData,
        markerType: IrType,
        markerConstructor: IrConstructorSymbol,
        fillerPlan: FillerPlan,
        config: CaptureCodePluginConfig,

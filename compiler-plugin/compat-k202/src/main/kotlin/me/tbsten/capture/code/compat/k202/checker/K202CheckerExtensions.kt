...
/**
 * Kotlin 2.0.21 baseline 向けの **FIR Checker 群 + FirAdditionalCheckersExtension 群**。
 *
 * task-124 で旧 4 checker file (`K202CaptureCodeMarkerClassChecker.kt` /
 * `K202MarkerAnnotationChecker.kt` / `K202CapturedSourcesCallChecker.kt` /
 * `K202ExpressionSiteCollector.kt`) を本 file に集約。
 *
 * task-119 follow-up: 各 checker で `CompatContextImpl()` を 3 個 new していた状態を
 * module-scoped singleton ([k202Compat]) で共有する形に統一。
 */
...
internal object K202MarkerAnnotationChecker : FirRegularClassChecker(MppCheckerKind.Common) {
    ...
    override fun check(
        ...
        logic(context, reporter, declaration, k202Compat, diagnostics)
...
internal object K202CapturedSourcesCallChecker : FirExpressionChecker<FirFunctionCall>(MppCheckerKind.Common) {
    ...
    private val diagnostics = object : ValidateCapturedSourcesCall.Diagnostics {
        ...
        override val capturedSourcesTIsTypeParameter: KtDiagnosticFactory1<String> =
            CompatContextImpl.K202Diagnostics.CC_CAPTUREDSOURCES_T_IS_TYPE_PARAMETER
    }
    ...
    override fun check(
        ...
        logic(context, reporter, expression, k202Compat, diagnostics)
    }
}

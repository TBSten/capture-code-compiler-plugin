...
/**
 * Logic C (design §5.C) のソーステキスト取得本体。Kotlin 2.0.0 (`compat-k2020`) 向け実装。
 *
 ...
 * ## 配置の設計判断
 *
 * 本 extractor は `IrFile` (= IR API) を直接受け取るため `:compat-k202` 配下に置く方が依存関係
 * としても自然 (`:compiler-plugin` は `:compat-k202` に依存しているため、逆方向に IR API を
 * 持ち込むと循環依存になりかねない)。
 *

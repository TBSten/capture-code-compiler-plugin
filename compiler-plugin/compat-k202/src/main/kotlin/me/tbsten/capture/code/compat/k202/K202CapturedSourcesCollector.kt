 ...
 * compat-k202 内部だけで使う internal な data class として切り出している (SSOT は CapturedSite、
 * markerCall はあくまで collector → rewriter のローカル受け渡し情報)。
 */
internal data class K202CapturedSiteData(
     ...
     * [me.tbsten.capture.code.compat.k202.userargs.UserArgIrBuilder.buildOrDefault] の
 ...
 */
internal class K202CapturedSourcesCollector(
    ...
    fun collectFileAnnotations() {
        ...
        for ((markerFqn, markerCall) in fileAnnotations) {
            ...
            capturedSiteData += K202CapturedSiteData(
                site = CapturedSite(
                    markerFqn = markerFqn,
                    source = source,
     ...
     * `endLine`) も合わせて埋める。後段 ([K202CapturedSourcesRewriter]) で
    ...
    private fun skipLeadingAnnotationLines(text: String, startOffset: Int, endOffset: Int): Int {
        ...
        while (cursor < endOffset) {
            ...
            if (cursor < endOffset && text[cursor] == '(') {
                ...
                // balanced 取れなかった場合 (= depth > 0 のまま endOffset に達した) は安全側で
                // 元の lineStart を返して再構成は normalizer に委ねる
                if (depth != 0) return lineStart
            }
            // 4. annotation 直後の trailing whitespace を吸収。
            //    改行に到達した場合は改行も 1 つだけ消費し、 次の token を新しい行頭として扱う
            //    (次イテレーションの lineStart が新しい行頭になる)。

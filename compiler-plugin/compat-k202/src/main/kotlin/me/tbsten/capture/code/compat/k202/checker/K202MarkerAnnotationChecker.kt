 ...
 *
 * task-072 で `:compiler-plugin` main module の `MarkerAnnotationChecker` を compat-k202 layer に
 ...
 * 2.2.x runtime で AbstractMethodError を起こす。 compat-k202 module 自体が **常に 2.0.0 baseline**
 * (`kotlin-compiler-embeddable-k202` ref) で compile されるため、 ServiceLoader 経由で
 * compat-k202 が選択された環境では runtime も 2.0.x 〜 2.0.21 までと整合する。
 *
...
internal object K202MarkerAnnotationChecker : FirRegularClassChecker(MppCheckerKind.Common) {
    ...
    override fun check(
        ...
        if (declaration.isExpect) {
            reporter.reportOn(source, K202CaptureCodeDiagnostics.CC_MARKER_IS_EXPECT, context)
        }
    ...
    private fun checkVisibility(
        ...
        if (visibility != Visibilities.Internal && visibility != Visibilities.Private) {
            reporter.reportOn(
                declaration.source,
                K202CaptureCodeDiagnostics.CC_MARKER_VISIBILITY_VIOLATION,
                context,
            )
        }
    }
    ...
    private fun checkRetention(
        ...
        if (declaration.getRetention(context.session) != AnnotationRetention.SOURCE) {
            reporter.reportOn(
                declaration.source,
                K202CaptureCodeDiagnostics.CC_MARKER_RETENTION_VIOLATION,
                context,
            )
        }
    }
    ...
    private fun checkTargetSites(
        ...
        if (targetAnnotation == null) {
            reporter.reportOn(
                declaration.source,
                K202CaptureCodeDiagnostics.CC_MARKER_TARGET_EMPTY,
                context,
            )
            return
        }
    ...
    private fun checkParameters(
        ...
        for (parameterSymbol in primaryConstructor.valueParameterSymbols) {
            ...
            // path inside compat-k202 (which gets selected for every 2.0.x consumer).

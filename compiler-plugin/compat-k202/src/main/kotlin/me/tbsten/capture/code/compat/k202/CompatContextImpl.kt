...
public class CompatContextImpl : CompatContext {
    ...
    // -- task-0.2.0-cifix: FIR type API drift dispatchers (K2.0.20-21 baseline) --
    //
    // 2.0.21 baseline でも `FirTypeRef.coneType`, `FirTypeRef.coneTypeOrNull`,
    // `FirExpression.resolvedType`, `ConeKotlinType.classId` extension は同じ
    // `org.jetbrains.kotlin.fir.types` package に存在し、 K2.0 と同形で呼べる。
    ...
    override fun diagnosticFactory(id: String): DiagnosticFactoryRef? = K202Diagnostics.MAP[id]

    // -- task-120-B Phase 2: IR primitive overrides (K2.0.21 baseline) --
    //
    // K2.0.21 でも `IrCallImpl` 6 引数 ctor / 旧 `putValueArgument` 等が引き続き使える
    // (compat-k202 が kotlin-compiler-embeddable 2.0.21 baseline で compile される)。
    // K2.1 で internal 化される `IrCallImpl(...)` ctor も baseline では public のまま。
    ...
    override fun walkIrTree(
        ...
        moduleFragment.acceptChildrenVoid(
            K202CallbackVisitor(onClass, onSimpleFunction, onProperty, onTypeAlias),
        )
    }
    ...
    override fun transformCallsInModule(
        ...
        moduleFragment.transformChildrenVoid(K202CallTransformer(onCall))
    }
    ...
    override fun transformCallsInFile(
        ...
        file.transformChildrenVoid(K202CallTransformer(onCall))
    }
    ...
    override fun newIrCall(
        ...
        // K2.0.21 baseline: `valueArgumentsCount` is still on the ctor signature.
    ...
    // -- task-0.2.0-cifix-ir: IR factory drift dispatchers (K2.0.20-21 baseline) --
    //
    // K2.0.20+ で旧 `IrVarargImplKt` / `IrConstImplKt` / `IrGetEnumValueImplKt` top-level
    // host class が削除され、 各 builder は `IrVarargImpl.kt` 内の top-level fn として
    // 同じく `IrVarargImplKt` 相当の場所に置かれているが、 `BuildersKt` の存在も加わり
    // main bytecode の reference が host class shape drift する点を吸収する。
    // 2.0.21 baseline では既に top-level builders が正規ルートなので、 そのまま呼べる。
    ...
    override fun newIrConstPrimitive(
        ...
        // K2.0.21 baseline: `IrConstKind<T>` (generic) は K2.0 と同形。
        startOffset = startOffset,
        endOffset = endOffset,
        type = type,
        kind = kind as IrConstKind<Any?>,
        value = value,
    )
    ...
    public object K202Diagnostics {
        ...
        // task-135: IR rewrite phase silent fall back の warning factories。

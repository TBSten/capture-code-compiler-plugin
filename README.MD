...
> Until it reaches version 1.0.0, it may contain many breaking changes and unintended bugs.
...
// 1. Define an annotation and mark it with @CaptureCode
@CaptureCode
...
// 2. Put that annotation on the code you want to capture
@Snippet fun greet() = "Hello!"
@Snippet fun farewell() = "Goodbye!"

fun main() {
    // 3. Read the captured source back as a string with capturedSources
...
// a block of statements, without any wrapper in the captured text
...
### Capturing a block of statements

Two adjacent statements are not a single Kotlin expression, so they cannot be annotated as one.
`runWithCaptureCode(Marker::class) { ... }` captures the **body of the block only** — the
`runWithCaptureCode(...) {` header and the closing `}` are not part of the captured text:
...
It runs the block and returns its value, just like `run { ... }`:
...
Compared with annotating an expression, this form:
...
- is a plain function call, so it is not subject to the `@Marker()` empty-argument-list
  requirement forced by the K2 parser (see Known limitations),
- leaves no `run { ... }` wrapper in the captured text.

Marker parameters cannot be supplied at the call site; markers used this way should have a
default value for every non-filler parameter. Without the compiler plugin applied the block
...
:compiler-plugin                // FIR + IR extensions
  ...
  :compiler-plugin:compat-k200  // Kotlin 2.0.0 / 2.0.10 impl
  :compiler-plugin:compat-k202  // Kotlin 2.0.20 / 2.0.21 impl
  :compiler-plugin:compat-k210  // Kotlin 2.1.x impl
  :compiler-plugin:compat-k220  // Kotlin 2.2.x impl
  :compiler-plugin:compat-k230  // Kotlin 2.3.x impl
  :compiler-plugin:compat-k240  // Kotlin 2.4.x impl
...
| ------ | ------ | ------------- |
| 2.0.0 / 2.0.10 | Supported (verified in CI) | `compat-k200` |
| 2.0.20 / 2.0.21 | Supported (verified in CI) | `compat-k202` |
| 2.1.x  | Supported (verified in CI) | `compat-k210` |
| 2.2.x  | Supported (verified in CI) | `compat-k220` |
| 2.3.x  | Supported (verified in CI) | `compat-k230` |
| 2.4.0 / 2.4.10 / 2.4.20-RC | Supported (verified in CI) | `compat-k240` |
| ≥ 2.4.20 (stable) | Not yet verified — dispatches to `compat-k240` with a warning | — |
...
6. Incremental compilation is disabled in modules that apply the Gradle
   plugin (a stale IC round would break the rewrite; opt out via
   `captureCode { disableIncrementalCompilation = false }`).
